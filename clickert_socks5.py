...
def down(indirme,api="https://api.proxyscrape.com/?request=getproxies&proxytype=socks5&timeout=10000&country=all&ssl=all&anonymity=all"):
    if not api:
        print("\t\tGörünüşe göre api kısmını boş bırakmışsın . Orjinal api ile devam ediliyor!")
        api="https://api.proxyscrape.com/?request=getproxies&proxytype=socks5&timeout=10000&country=all&ssl=all&anonymity=all"
    if not indirme:
        print("\t\tApi kaynağından güncel liste indiriliyor")
        try:
            req.urlretrieve(api,"socks5.txt")
        except:
...
def linktl(agents,page_load_timeout=40):
    while True:
        ...
        while True:    
            try:
                ...
                try:
                    ...
                    print("\n\t\tsocks5.txt bulunamadı . Api kaynağından indiriliyor !")
                    down(indirme,api)      ######socks5.txt YOK NE YAPACAĞIZ ?
        ...
        chrome_options.add_argument('--proxy-server=socks5://{}'.format(liste))
                ...
                try:
                    ...
                    with open("socks5.txt") as liste:
                        ...
                        if not liste:
                            ...
                            break
                except:print("\n\t\tsocks5.txt bulunamadı . Api kaynağından indiriliyor !");down(indirme,api) 
...
######################################################
try:
    with open("settings4socks5.json") as settings:
        settings = settings.read()
        settings = json.loads(settings)
        agents = settings["user-agent"]
        api = settings["api"]
except FileNotFoundError:
    print("\n\t\t'settings4socks5.json' ayar dosyası bulunamadı ! Oluşturuluyor . Lütfen gerekli ayarları yapınız !\n")
    with open("settings4socks5.json","w") as settings:
        settings.write(orj_json)
    ...
    if ikinci_secim == 2:
        try:
            ...
            if not deg_link:
                ...
                with open("settings4socks5.json","w") as dosya:
