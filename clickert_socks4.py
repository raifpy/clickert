...
import base64
import random
#win7'de renk kodlarımız çalışmıyor
#
# Evet socks4 ve socks5 seçeneğini direk clickert.py 'in içine gömebilirdim . Lakin bunu yapmak için en az 2 saatimi harcamam gerekiyor . Bu şekilde de iş görüyor ..
import platform
import threading
import urllib
...
def down(indirme,api="https://api.proxyscrape.com/?request=getproxies&proxytype=socks4&timeout=10000&country=all"):
    if not api:
        print("\t\tGörünüşe göre api kısmını boş bırakmışsın . Orjinal api ile devam ediliyor!")
        api="https://api.proxyscrape.com/?request=getproxies&proxytype=socks4&timeout=10000&country=all"
    if not indirme:
        print("\t\tApi kaynağından güncel liste indiriliyor")
        try:
            req.urlretrieve(api,"socks4.txt")
        except:
...
def linktl(agents,page_load_timeout=40):
    while True:
        ...
        while True:    
            try:
                ...
                try:
                    ...
                    print("\n\t\tsocks4.txt bulunamadı . Api kaynağından indiriliyor !")
                    down(indirme,api)      ######socks4.txt YOK NE YAPACAĞIZ ?
        ...
        chrome_options.add_argument('--proxy-server=socks4://{}'.format(liste))
                ...
                try:
                    ...
                    with open("socks4.txt") as liste:
                        ...
                        if not liste:
                            ...
                            break
                except:print("\n\t\tsocks4.txt bulunamadı . Api kaynağından indiriliyor !");down(indirme,api) 
...
######################################################
try:
    with open("settings4socks4.json") as settings:
        settings = settings.read()
        settings = json.loads(settings)
        agents = settings["user-agent"]
        api = settings["api"]
except FileNotFoundError:
    print("\n\t\t'settings4socks4.json' ayar dosyası bulunamadı ! Oluşturuluyor . Lütfen gerekli ayarları yapınız !\n")
    with open("settings4socks4.json","w") as settings:
        settings.write(orj_json)
    ...
    if ikinci_secim == 2:
        try:
            ...
            if not deg_link:
                ...
                with open("settings4socks4.json","w") as dosya:
